...
from .awfln_relates import SoftAttn

class Net(nn.Module):
#class AWFLN(nn.Module):
    #self, num_channels, base_filter, args
    def __init__(self, num_channels, base_filter, args):
    #def __init__(self, scale_factor):
        print("Net _init_\n")
        #print(args)

        scale_factor = args['data']['upsacle']
    ...
    #######################
    def forward(self, ms_org, ms_up, pan):

        ms_org_up = self.up_sample4(ms_org)  ## in_channels, in_channels * up_scale ** 2
        #print("lms_image shape:", lms_image.shape)
       # print("bms_image shape:", bms_image.shape)
        #print("pan_image shape:", pan_image.shape)

        #print("ms_up shape:", ms_up.shape)  # 确保 ms_up 正确生成
        #print("ms_org shape:", ms_org.shape)  # 确保 ms_org_up 正确生成
        #print("ms_org_up shape:", ms_org_up.shape)  # 确保 ms_org_up 正确生成
        #print("pan shape:", pan.shape)  # 确保 pan 的形状正确

        data1 = torch.cat([ms_org_up, pan], dim=1)
        # ms_up4 = self.up_sample2(ms_up2)
        pan_conv = self.blk_5_30_3(data1)
        #print("pan_conv shape:",pan_conv.shape)
        ms_up_conv = self.blk_4_30_3(ms_up)
        #print("ms_up_conv shape:",ms_up_conv.shape)
