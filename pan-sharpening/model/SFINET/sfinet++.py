...
class Net(nn.Module):
    ...
    def forward(self,l_ms,bms,pan):
        ...
        #print(f"m,n shape: {m,n}")
        
        mHR = upsample(l_ms, M, N)
        #print(f"mHR shape: {mHR.shape}")
        l_ms_2 = upsample(l_ms, m*4, n*4)
        #print(f"l_ms_2 shape: {l_ms_2.shape}")
        pan_4 = upsample(pan, m, n)
        #print(f"pan_4 shape: {pan_4.shape}")
        pan_2 = upsample(pan, m*4, n*4)
        #print(f"pan_2 shape: {pan_2.shape}")

        HRf_4 = self.process1(l_ms, pan_4)
        #print(f"HRf_4 shape: {HRf_4.shape}")
        HRf_2 = upsample(HRf_4, m*4, n*4)
        #print(f"HRf_2 shape: {HRf_2.shape}")
        HRf_2 = self.process1(l_ms_2, pan_2)+HRf_2
        #print(f"HRf_2 shape: {HRf_2.shape}")
        HRf = upsample(HRf_2, m*4, n*4)
        
        #print(f"mHR shape: {mHR.shape}")
        #print(f"pan shape: {pan.shape}")
        #print(f"self.process3(mHR, pan) shape: {self.process3(mHR, pan).shape}")
        #print(f"HRf shape: {HRf.shape}")
        
        #mHR shape: torch.Size([4, 4, 128, 128])
        #pan shape: torch.Size([4, 1, 128, 128])
        #self.process3(mHR, pan) shape: torch.Size([4, 4, 128, 128])
        #HRf shape: torch.Size([4, 4, 64, 64])
