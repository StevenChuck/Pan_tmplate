...
class MambaIRUNet(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        _, _, H, W = inp_img.shape #torch.Size([1, 3, 256, 256])

        inp_enc_level1 = self.patch_embed(inp_img)  # b,hw,c #inp_enc_level1 torch.Size([1, 65536, 24])
        # print(inp_enc_level1.shape,'come')
        out_enc_level1 = inp_enc_level1
        for layer in self.encoder_level1:
            out_enc_level1 = layer(out_enc_level1, [H, W])
        inp_enc_level2 = self.down1_2(out_enc_level1, H, W)  # b, hw//4, 2c  torch.Size([1, 16384, 48])
        print(" inp_enc_level2", inp_enc_level2.shape)
        inp_img1 = F.interpolate(inp_img, scale_factor=0.5)#torch.Size([1, 3, 128, 128])
        inp_img2 = F.interpolate(inp_img1, scale_factor=0.5)#torch.Size([1, 3, 64, 64])
        inp_img3 = F.interpolate(inp_img2, scale_factor=0.5)#torch.Size([1, 3, 32, 32])
...
if __name__ == '__main__':
    ...
    # 设置参数（例如 hidden_dim = 48，与 VSSBlock 初始化时的默认值一致）
    hidden_dim = 24
    B, H, W = 2, 128, 128  # 批量大小 1，图像尺寸 32x32
    L = H * W  # 展平后的长度

    # 构造一个随机输入，形状为 [B, L, hidden_dim]
    dummy_input = torch.randn(B, L, hidden_dim).to(device)
    x_size = (H, W)

    # 实例化 VSSBlock
    vss_block = VSSBlock1(hidden_dim=hidden_dim, drop_path=0.1, d_state=16, expand=2.0)
    vss_block = vss_block.to(device)

    # 进行前向传播
    output = vss_block(dummy_input, x_size)
