...
def nan_hook(module, input, output):
    if not isinstance(output, torch.Tensor):
        return  # 有些模块可能返回 tuple，比如 attention
    if torch.isnan(output).any():
        print(f"[NaN detected] in {module.__class__.__name__}")
        print(f"Input range: {[x.min().item() if isinstance(x, torch.Tensor) else None for x in input]}")
        print(f"Output min: {output.min().item()}, max: {output.max().item()}")
        raise RuntimeError("NaN encountered! Stopping forward pass.")
    if torch.isinf(output).any():
        print(f"[Inf detected] in {module.__class__.__name__}")
        print(f"Input range: {[x.min().item() if isinstance(x, torch.Tensor) else None for x in input]}")
        print(f"Output min: {output.min().item()}, max: {output.max().item()}")
        raise RuntimeError("Inf encountered! Stopping forward pass.")

# 给模型加 hook
def add_nan_hooks(model):
    for name, layer in model.named_modules():
        layer.register_forward_hook(nan_hook)
    print("✅ NaN/Inf debug hooks added.")

# 用法示例
# 假设你的模型是 Net()
# net = Net()
# add_nan_hooks(net)
# output = net(input)  # 如果某一层出现 NaN/Inf，就会立刻报错并打印位置
...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        add_nan_hooks(self.model)  # 添加 NaN/Inf 检测 hook
    ...
    def train(self): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            epoch_loss = 0
            scaler = torch.amp.GradScaler("cuda") if self.cuda else torch.amp.GradScaler("cpu")
...
#                 print(Variable(batch[0]))
    ...
    #                 print(lms_image.shape,bms_image.shape,pan_image.shape)
                    ...
                    if self.cfg['schedule']['use_YCbCr']:
                        ...
                        loss = loss + 0.5* vgg_loss  # 将 VGG Loss 加入总损失
                ...
                if self.cfg['schedule']['gclip'] > 0:
                    scaler.unscale_(self.optimizer)  # 先解缩放再裁剪
